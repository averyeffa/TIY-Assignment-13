...
// INPUT: arrayOfObj and a filter Obj for valid props/values
// OUTPUT: array of objects, filtered set
...
var where = function(arrOfObjects, criteriaObj){
  ...
  for (var i = 0; i < arrOfObjects.length; i++) {
    ...
    var singleObj = arrOfObjects[i];
    var objectMatchesCriteria = true;

    for (var propStr in criteriaObj) {
      if (criteriaObj [propStr] !== singleObj[propStr]) {
        objectMatchesCriteria = false;

      }
    }
    if (objectMatchesCriteria) {
      filteredListOfObjects.push(singleObj);
    }
  }
